...
class Database(Database):
    ...
    def CreateSubset( self ):
        ...
        def Chunkify(li):
            return [ li[i:i+150] for i in range(0, len(li), 150) ]
...
class Calculator(object):
    ...
    def DoValidation(self):
        ...
        self._dumpdir = os.path.join(self._outdir, 'dumpdir')

        if not os.path.exists(self._dumpdir):
    ...
    def DoCalc(self, selection, tree, namedEntries, status, iteration = 0):
        ...
        for entry in namedEntries:
            name = '.'.join( map(str, tree.GetPart(entry, -1) ))
            nameFrequencies[ name ] = nameFrequencies.get(name, 0)+1
            # nameFrequencies[ tree.GetStrName(tree.GetPart(entry, Tree.DEPTH-1)) ] = nameFrequencies.get( tree.GetStrName(entry), 0) + 1
        ...
        for entry in tqdm( selection, desc=status ):
            ...
            if tree.HasName( entry._key ) and entry._key not in namedEntries:
                ...
                name = '.'.join(map(str, tree.GetPart(entry._key, -1)))
                nameFrequencies[ name ] = nameFrequencies.get( name, 0 ) + 1
        ...
        if iteration == 0:
            for entry in namedEntries:
                ...
                self._entryBase.GetEntry( entry )._wgstHistory.append( tree.GetStrName( entry ) )
        ...
        for entry in namedEntries:
            ...
            if oldName:
                partial_name = '.'.join( oldName.split('.')[:-1] )

            if oldName == newName:
                continue
                
            if iteration > 0 and oldName and nameFrequencies[partial_name] > 1:
        ...
        out_name = 'freq_dump_{}'.format(datetime.now().strftime('%m-%d-%Y@%H-%M-%S'))
        out_path = os.path.join(self._dumpdir, out_name )

        with open(out_path, 'w') as f:
            json.dump(nameFrequencies, f)
...
def Main(args):
    ...
    try:
        ...
        outdir = os.path.join( args['outdir'], 'state_data' )
