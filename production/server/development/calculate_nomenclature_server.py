...
def CalcName(named, tree, unNamedEntry, distances, thresholds ):
    ...
    for level, threshold in enumerate( thresholds, 1 ):
        ...
        for i, (dist, namedIndex) in enumerate( distances ):
            ...
            if dist <= threshold:
                keyToGet = namedEntries[namedIndex]
...
class Calculator(object):
    ...
    def DoCalc( self, args ):
        ...
        def CheckCore(entryExper):
            ...
            if float(present) / float(len(self._charNrs)) < self._minPres:
                return None
        ...
        for i, entry in enumerate( Database.Db.Selection ):
            ...
            for i in range(len(namedEntries)):
                dist = GetDistance( eCalls, self._alleleCalls.GetCalls(namedEntries[i]))
        ...
        if GB_PARAMS['nosave']:
            ...
            with open( FILE_PATH, 'wb' ) as f:
                ...
                for key in belowQC:
                    if key in Database.Db.Selection:
